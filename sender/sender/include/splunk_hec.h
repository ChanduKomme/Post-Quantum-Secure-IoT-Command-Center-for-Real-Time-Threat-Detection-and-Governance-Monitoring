...
/*
 * splunk_hec.h — Auto-discovering Wi-Fi → Splunk HEC sender (SENDER role)
 * ─────────────────────────────────────────────────────────────────────────
 * The BL602 sender board discovers the Splunk server automatically via
 * UDP broadcast — no hardcoded IP address anywhere.
 *
 * HOW IT WORKS:
 *   1. On first event, board broadcasts "PQC_SPLUNK_DISCOVER" on UDP 9998
 *   2. PC runs: python3 integrations/splunk_hackerone_jira/splunk/splunk_discovery_server.py
 *   3. Server responds: "PQC_SPLUNK_HOST:<pc-ip>:8088"
 *   4. Board caches IP and sends all HEC events directly over Wi-Fi
 *
 * OVERRIDE (optional — skip discovery, use fixed IP):
 *   Add to bouffalo.mk:  CFLAGS += -DSPLUNK_STATIC_HOST=\"192.168.1.x\"
 */
...
#define SPLUNK_DEVICE     "sender-01"
#define SPLUNK_ROLE       "sender"
...
#define SPLUNK_DISCOVER_TIMEOUT  3
#define SPLUNK_DISCOVER_TRIES    3
#define SPLUNK_CONN_TIMEOUT      4
...
/*
 * splunk_discover() — UDP broadcast on LAN to find the Splunk server.
 * Works on any network/subnet automatically.
 */
static inline bool splunk_discover(void)
{
    ...
    printf("[Splunk-S] Using static host: %s:%s\r\n",
    ...
    for (uint8_t attempt = 0; attempt < SPLUNK_DISCOVER_TRIES; ++attempt) {
        ...
        if (sock < 0) {
            printf("[Splunk-S] discover: socket() failed\r\n");
            continue;
        }
        ...
        if (bind(sock, (struct sockaddr *)&local, sizeof(local)) < 0) {
            printf("[Splunk-S] discover: bind() failed\r\n");
            close(sock);
            continue;
        }

        const char *msg = SPLUNK_DISCOVER_MSG;
        ...
        printf("[Splunk-S] discover: broadcast attempt %u sent\r\n",
               (unsigned)(attempt + 1));
        ...
        if (n <= 0) {
            printf("[Splunk-S] discover: no response (attempt %u)\r\n",
                   (unsigned)(attempt + 1));
            continue;
        }
        ...
        if ((size_t)n < prefix_len ||
            ...
            printf("[Splunk-S] discover: unexpected response\r\n");
            continue;
        }
        ...
        if (colon) {
            size_t ip_len = (size_t)(colon - rest);
            if (ip_len == 0 || ip_len >= sizeof(g_splunk_host)) continue;
        ...
        if (g_splunk_host[0] == '\0') continue;

        printf("[Splunk-S] Auto-discovered Splunk at %s:%s\r\n",
               g_splunk_host, g_splunk_port);
        return true;
    }

    printf("[Splunk-S] Auto-discovery failed after %d attempts\r\n",
           SPLUNK_DISCOVER_TRIES);

    /* Automatic fallback for laptop-hotspot mode: use Wi-Fi gateway IP. */
    if (splunk_get_default_gateway_host(g_splunk_host, sizeof(g_splunk_host))) {
        strncpy(g_splunk_port, SPLUNK_HEC_PORT, sizeof(g_splunk_port) - 1);
        printf("[Splunk-S] Using Wi-Fi gateway as Splunk host: %s:%s\r\n",
               g_splunk_host, g_splunk_port);
        return true;
    }

    printf("[Splunk-S] No automatic Splunk host found. Use laptop hotspot or enable UDP discovery.\r\n");
    return false;
#endif
}
...
/*
 * splunk_send_event() — send one JSON event to Splunk HEC over Wi-Fi.
 * Auto-discovers the server on first call.
 * Re-discovers after 5 consecutive failures (handles IP changes).
 */
    ...
    const char *action)
{
    ...
    if (!splunk_token_configured()) {
        if (!token_warned) {
            printf("[Splunk-S] disabled: SPLUNK_HEC_TOKEN not set at build time\r\n");
            token_warned = true;
        }
        return;
    }
    ...
    if (g_splunk_fail_count >= SPLUNK_MAX_CONSECUTIVE_FAILS) {
        printf("[Splunk-S] Re-discovering after %lu failures\r\n",
    ...
    int body_len = snprintf(body, sizeof(body),
        "{"
          ...
          "}"
        "}",
        SPLUNK_INDEX, SPLUNK_SOURCE, SPLUNK_SRCTYPE,
        event_type, severity, action,
        SPLUNK_DEVICE, SPLUNK_ROLE);
    ...
    if (req_len <= 0 || req_len >= (int)sizeof(request)) return;
    ...
    if (getaddrinfo(g_splunk_host, g_splunk_port, &hints, &res) != 0 || !res) {
        printf("[Splunk-S] DNS resolve failed for %s\r\n", g_splunk_host);
        g_splunk_fail_count++;
        return;
    }
    ...
    if (sock < 0) { freeaddrinfo(res); g_splunk_fail_count++; return; }
    ...
    if (connect(sock, res->ai_addr, res->ai_addrlen) != 0) {
        printf("[Splunk-S] Connect failed to %s:%s\r\n",
               g_splunk_host, g_splunk_port);
    ...
    if (strstr(resp, "200") || strstr(resp, "\"code\":0")) {
        printf("[Splunk-S] OK -> %s [%s]\r\n", event_type, severity);
        g_splunk_fail_count = 0u;
    } else {
        printf("[Splunk-S] WARN -> %.80s\r\n", resp);
        g_splunk_fail_count++;
    }
}
